<?php
require_once __DIR__ . '/../../../db/database.php';
require_once __DIR__ . '/../../util/api_helpers.php';

if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    respond(405, "Method not allowed");
}

$data = readJsonBody();

$request_id = $data["request_id"] ?? null;
$friend1_id = $data["friend1_id"] ?? null;
$other_username = trim($data["friend_username"] ?? "");
$other_user_id = $other_username !== "" ? getUserIdByUsername($other_username) : ($data["friend2_id"] ?? ($data["friend_id"] ?? null));

if (!$request_id && (!$friend1_id || ($other_username === "" && !$other_user_id))) {
    respond(400, "Missing friend username");
}

if (!$request_id && $other_username !== "" && !$other_user_id) {
    respond(404, "User not found");
}

if ($request_id) {
    if (!$friend1_id) {
        respond(400, "Missing current user ID");
    }

    $stmt = $db->prepare("
        DELETE FROM friends
        WHERE id = :request_id
          AND friend2_id = :friend1_id
          AND status = 'pending'
    ");

    $stmt->execute([
        ":request_id" => $request_id,
        ":friend1_id" => $friend1_id
    ]);
} else {
    $stmt = $db->prepare("
        DELETE FROM friends
        WHERE friend1_id = :other_user_id
          AND friend2_id = :friend1_id
          AND status = 'pending'
    ");

    $stmt->execute([
        ":other_user_id" => $other_user_id,
        ":friend1_id" => $friend1_id
    ]);
}

if ($stmt->rowCount() > 0) {
    respond(200, "Friend request refused");
}

if ($request_id) {
    $check = $db->prepare("
        SELECT status, friend2_id FROM friends
        WHERE id = :request_id
        LIMIT 1
    ");

    $check->execute([":request_id" => $request_id]);
} else {
    $check = $db->prepare("
        SELECT status, friend2_id FROM friends
        WHERE friend1_id = :other_user_id
          AND friend2_id = :friend1_id
        LIMIT 1
    ");

    $check->execute([
        ":other_user_id" => $other_user_id,
        ":friend1_id" => $friend1_id
    ]);
}

$friendship = $check->fetch(PDO::FETCH_ASSOC);

if (!$friendship) {
    respond(404, "Friend request not found");
}

if ($friendship["friend2_id"] !== $friend1_id) {
    respond(403, "Only the request receiver can refuse this friend request");
}

respond(409, "Friend request is not pending");
